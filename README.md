...
- Maximum active documents per run: 10
- PDF pages per document: adaptive 80-page normal mode, 40-page fallback mode
...
The workflow has a concurrency lock and a 20-minute job timeout. Adaptive throughput state is retained only inside the authenticated encrypted state asset. A prior timed-out workflow immediately selects 40 pages; two consecutive completed runs longer than 15 minutes also select 40. Three consecutive runs of 10 minutes or less restore 80 pages. Its token can update only this private worker repository's encrypted release-state asset. It cannot write to The Crucible.
