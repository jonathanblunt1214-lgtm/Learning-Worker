...
Each run treats the newly verified vetted-custody queue and learning envelope as authoritative. Prior encrypted worker state is decrypted into a separate directory; only extraction progress for the same source id and exact content hash, plus candidate-only records with identical duplicate bodies, may be merged forward. Removed sources and changed content remain excluded. Advanced learning records and knowledge versions are preserved verbatim in an additive encrypted quarantine artifact and cannot enter the active worker store.
