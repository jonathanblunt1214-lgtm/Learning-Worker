...
const DEFAULT_STATE = Object.freeze({ schemaVersion:1, pagesPerDocument:70, consecutiveLongRuns:0, consecutiveHealthyRuns:0, lastDurationSeconds:null, lastOutcome:null, updatedAt:null });
function validate(value) {
  ...
  if (extras.length || value.schemaVersion !== 1 || ![35, 70].includes(value.pagesPerDocument)) throw new Error('Adaptive throughput state is invalid.');
...
function read(file) {
  ...
  if (parsed && parsed.schemaVersion === 1 && [40,80].includes(parsed.pagesPerDocument)) parsed.pagesPerDocument=parsed.pagesPerDocument === 80 ? 70 : 35;
...
function plan(state, { previousConclusion, now = new Date().toISOString() } = {}) {
  ...
  if (previousConclusion === 'timed_out') { next.pagesPerDocument=35; next.consecutiveLongRuns=0; next.consecutiveHealthyRuns=0; next.lastOutcome='timed_out'; next.updatedAt=now; }
  return next;
}
function complete(state, durationSeconds, now = new Date().toISOString()) {
  ...
  if (durationSeconds > 900) { next.lastOutcome='long'; next.consecutiveLongRuns += 1; next.consecutiveHealthyRuns=0; if (next.consecutiveLongRuns >= 2) next.pagesPerDocument=35; }
  else { next.lastOutcome='healthy'; next.consecutiveLongRuns=0; next.consecutiveHealthyRuns += 1; if (next.pagesPerDocument === 35 && durationSeconds <= 600 && next.consecutiveHealthyRuns >= 3) { next.pagesPerDocument=70; next.consecutiveHealthyRuns=0; } }
  return next;
}
